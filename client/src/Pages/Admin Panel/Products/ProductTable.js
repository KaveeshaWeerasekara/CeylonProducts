...
const ProductTable = (filter) => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      setLoading(true);
      const category = searchedparams.get("category") || "";
      if (category != "all") {
        const body = {
          catergory: category,
        };
        const res = await axios.post("/api/users/getuserByCatergory", body); //fectch users by catregory
        if (res.data.success) {
